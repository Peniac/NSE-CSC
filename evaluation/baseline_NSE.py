'''
heuristic that embeds a consuming service, without optimization for the CSC links
...
def NSE(datacenter, service):
	nodes = [n for n in service.nodes if service.nodes[n]['sertype'] == 'consumer']
	sequence = {nodes[i+1]:nodes[i] for i in range(len(nodes)-1)}
	...
	# place the first node in the sorted racks, in the most empty server
	placed = False
	for tor in tors:
		servers = [s for s in datacenter.allServers() if s.tor == tor]
		servers.sort(key = lambda x: x.cpu, reverse = True)
		for s in servers:
			if s.canFitNode(service, nodes[0]):
				s.putNode(service,nodes[0])
				datacenter.VNF_to_server_mapping[nodes[0]] = s
				placed = True
				break	
		if placed:
			break
